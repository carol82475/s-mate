...
export class TripsService {
  ...
  async createTrip(
    ...
    const itinerary = this.generateMockItinerary(
      destination,
      start_date,
      end_date,
      preferences || [],
    ...
    for (const day of itinerary) {
      const dayData =
        await this.repository.createItineraryDay({
          trip_id: trip.id,
      ...
      const checkpoints = day.checkpoints.map(
        (cp) => ({
      ...
      await this.repository.createCheckpoints(
        checkpoints,
      );
    }

    return await this.repository.findByIdAndUserId(
      trip.id,
      userId,
    );
  }
  ...
  async updateTrip(
    ...
    const trip =
      await this.repository.updateTrip(
        tripId,
        userId,
        updates,
      );
  ...
  async deleteTrip(
    ...
    const deleted =
      await this.repository.deleteTrip(
        tripId,
        userId,
      );
  ...
  async updateCheckpoint(
    ...
    const dayData = trip.itinerary_days.find(
      (d) => d.day_number === day,
    );
    ...
    const checkpoint = dayData.checkpoints.find(
      (c) => c.sort_order === checkpointIndex,
    );
    ...
    const dayIds = trip.itinerary_days.map(
      (d) => d.id,
    );

    const allCheckpoints =
      await this.repository.findCheckpointsByDayIds(
        dayIds,
      );
  ...
  private generateMockItinerary(
    ...
    const itinerary: ItineraryDayInput[] =
      [];
