...
class ForumService {
    constructor() {
        this.repository = new forum_repository_1.ForumRepository();
    }
    async createPost(userId, destination, content, images, dates) {
    ...
    async getPosts(sort = 'latest', page = 1, limit = 10) {
        ...
        const { posts, count } = await this.repository.findPosts(sort, offset, limit);
        return {
            posts,
            pagination: {
                page,
